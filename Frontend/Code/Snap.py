...
# Function to fetch Snapchat media files from the device and save them in separate folders based on type
def fetch_and_save_snapchat_media(output_folder):
    try:
        # Run the adb command to find Snapchat media files and iterate over each file path
        adb_command = 'adb shell "find /sdcard/DCIM/Snapchat -type f"'
        ...
        if stderr:
            ...
            print("Standard output:", stdout)  # Add this line to print the standard output
        ...
        print("Snapchat media files have been saved to:", output_folder)
    except Exception as e:
        print("An error occurred while fetching and saving Snapchat media:", e)
...
def get_media_type(file_path):
    ...
    if extension in ('.jpg', '.jpeg', '.png', '.gif'):
        return 'Images'
    elif extension in ('.mp4', '.mkv', '.avi', '.mov', '.gifv'):
        return 'Videos'
...
def select_output_folder():
    root = tk.Tk()
    root.withdraw()
    folder_path = filedialog.askdirectory(title="Select Folder to Save Snapchat Media")
    return folder_path
...
def main():
    try:
        ...
        if output_folder:
            print("Selected output folder:", output_folder)  # Add this line to print the selected output folder
            # Fetch and save Snapchat media files into appropriate folders
            fetch_and_save_snapchat_media(output_folder)
        else:
